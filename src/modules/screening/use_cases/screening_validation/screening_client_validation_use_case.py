...
- Uses `ScreeningProcessStepRepository.list_by_process()` which doesn't exist on BaseStepRepository
- `client_validation_notes` attribute name doesn't match the new `ClientValidationStep` model (field is just `notes`)
...
- `ClientValidationStepRepository.get_by_process_id()` instead of listing all steps
- Correct field names from the new model
"""
